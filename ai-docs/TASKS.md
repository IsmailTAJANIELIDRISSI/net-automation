...
- **Late-added LTAs = operator-launched, injected into the running monitor ✅** (2026-07-17): a folder dropped in mid-run is NOT auto-processed. Its card appears (watcher) and is **editable while other LTAs run** (fields disable on the card's own `isRunning || isDone`, not the global flag), so the operator fixes any wrong extracted value. Pressing **Lancer / Tout lancer** during an active run does NOT start a concurrent batch: the backend (`batchRunning`/`monitorActive` + `injectionQueue`, `tryQueueDuringActiveBatch`) queues the **edited ach object**, and the running Portnet monitor's `drainInjectionQueue()` submits it on the live sessions next poll cycle, skipping already-submitted/done LTAs (checkpoint). Uses the ach as-sent (no re-scan, so the fix isn't clobbered by re-extraction). Browsers never close. Declare-scellés (BADR) + delete stay globally gated to avoid BADR concurrency. (Supersedes the 2026-07-09 auto-injection: now operator-gated, edit-first.)
- **Live folder watcher ✅** (2026-07-09): `fs.watch` (non-recursive, 800 ms debounce) on the acheminements root — started in `folder:scan`, closed on quit — fires `acheminements-changed` (`preload.onAcheminementsChanged` → App.jsx `handleRefresh`) whenever a folder is added/removed at ANY time (idle or processing). Operator drops an LTA folder in → its card appears within ~1 s. (Editable once the current batch finishes; never auto-processed.) Non-recursive avoids a loop with the scan's nested `acheminement.json` writes. Backed by `scanSingleAcheminement`/`scanAcheminementsFolder` (extracted from folder:scan).
...
- **Scellés declaration for DUM Normale Partiel — human-gated ✅** (2026-06-09)
  ...
  - **Signed-serie input empty + declare mid-run ✅** (2026-08-20): the "Signature manuelle requise" input no longer pre-fills the validated `dumSerie` (operator types the serie shown in BADR after signing; dumSerie is placeholder-only), and is always editable + `autoFocus`; "Déclarer scellés" gated only on non-empty input (dropped `isGlobalRunning`). Declaring while a batch monitor is active is now **queued** (`declarationQueue` → `drainDeclarationQueue` in the monitor loop, `badrBusy`-guarded) so it never races the monitor's BADR work; runs directly when idle. `declareScellesPartielFlow` extracted from the IPC; `App.handleDeclareScelles` handles the queued result
