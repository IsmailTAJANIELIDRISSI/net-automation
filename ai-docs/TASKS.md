...
- **Auto re-scan drain loop ✅** (2026-07-09): `handleRunAll` (App.jsx) runs all launchable LTAs, then re-scans the folder and processes any new LTA folder dropped in during the run — repeating until a re-scan finds nothing new. Sessions are persistent (`sharedPortnetApp`/`sharedBadrConn`, only closed on quit), so late LTAs are picked up on the live BADR/Portnet sessions with no USB-cert / CAPTCHA re-entry. `computeLaunchable()`/`NON_LAUNCHABLE_PHASES` skip done/waiting/error/incomplete LTAs; ID-signature + 100-pass guards prevent loops. (A manual Pause button was prototyped then removed — it only stopped at LTA/poll boundaries, so the drain loop replaced it.)
- **Mid-monitoring pickup of new folders ✅** (2026-07-09): the drain loop only re-scans *after* a batch returns, but the long Portnet monitoring phase kept it blocked. So `monitorPendingPortnetRequests` (main.js) now re-scans each poll cycle via `injectNewlyAddedLtas()`: a newly-added folder that is **fully complete** is submitted on the live sessions and joins the polling set (consultation page reopened after); **incomplete** ones are logged once and re-checked next cycle (handled the moment their fields are filled). Backed by `scanSingleAcheminement`/`scanAcheminementsFolder` (extracted from folder:scan) + `acheminementMissingRequiredFields` (CommonJS mirror of requiredFields.js). Guarded by `badrBusy` so it doesn't collide with the BADR keepalive. A new folder also fires an `acheminements-changed` IPC event (`preload.onAcheminementsChanged` → App.jsx `handleRefresh`) so its **card appears live** during monitoring, not only after the batch ends.
- **Live folder watcher ✅** (2026-07-09): `fs.watch` (non-recursive, 800 ms debounce) on the acheminements root — started in `folder:scan`, closed on quit — fires `acheminements-changed` whenever a folder is added/removed at ANY time (idle or processing). Operator drops an LTA folder in → its card appears within ~1 s to fill/edit, no manual refresh. Non-recursive avoids a loop with the scan's nested `acheminement.json` writes.
